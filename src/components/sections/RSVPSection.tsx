...
const ContactCard: React.FC<{
  ...
  delay: string;
  active: boolean;
}> = ({ side, icon, delay, active }) => (
  <div
    className="rsvp-contact-card group"
    style={{ animation: active ? `fade-slide-up 0.5s ease-out ${delay} both` : 'none' }}
  >
    {/* Hover glow */}
    ...
    <div className="relative z-10">
      <span className="text-3xl mb-3 block" style={{ filter: 'drop-shadow(0 2px 6px hsl(var(--primary) / 0.2))' }}>{icon}</span>
      <p className="font-heading text-sm mb-0.5" style={{ color: 'hsl(var(--text-cream))' }}>{side}</p>
      <p className="font-ui text-[10px] text-muted mb-4">Family Contact</p>
      <div className="flex items-center justify-center gap-3">
        <a href="tel:+919999999999" className="rsvp-contact-link" aria-label={`Call ${side}`}>
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            <path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" />
          </svg>
        </a>
        <a href="https://wa.me/919999999999" target="_blank" rel="noopener noreferrer" className="rsvp-contact-link" aria-label={`WhatsApp ${side}`}>
